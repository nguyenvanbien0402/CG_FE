    ...
    <card-data-table>
      <template #table>
        <v-data-table
          ...
          height="750px"
          hide-actions
          class="relative h-process-no-search elevation-1 py-5 h-full row-height-50 table-border"
          @click:row="clickRow"
...
const LongTermNonReturnRepository = RepositoryFactory.get('longTermNonReturn')
const {t} = useI18n()
store.setBreadcrumb(store, {"name": t('nav.screen.longTermNonReturn'), "path": "long-term-non-return"})
...
const table = reactive({
  ...
  headers: [
    ...
    {
      title: t('longTermNonReturn.projectNo'),
      align: 'center',
      width: 150,
      sortable: false,
      key: 'projectNo'
    ...
    {
      title: t('longTermNonReturn.fullName'),
      align: 'center',
      width: 150,
      sortable: false,
      key: 'fullName'
    ...
    {
      title: t('longTermNonReturn.customerNameJp'),
      align: 'center',
      width: 150,
      sortable: false,
      key: 'customerNameJp'
    ...
    {
      title: t('longTermNonReturn.returnLp'),
      align: 'center',
      width: 120,
      sortable: false,
      key: 'returnLp'
    ...
    {
      title: t('longTermNonReturn.requestDate'),
      align: 'center',
      width: 120,
      sortable: false,
      key: 'requestDate'
    ...
    {
      title: t('longTermNonReturn.contactDate'),
      align: 'center',
      width: 120,
      sortable: false,
      key: 'contactDate'
    ...
    {
      title: t('longTermNonReturn.deliveryDate'),
      align: 'center',
      width: 120,
      sortable: false,
      key: 'deliveryDate'
    ...
    {
      title: t('longTermNonReturn.receiptDate'),
      align: 'center',
      width: 120,
      sortable: false,
      key: 'receiptDate'
    ...
    {
      title: t('longTermNonReturn.regDate'),
      align: 'center',
      width: 120,
      sortable: false,
      key: 'regDate'
...
const search = (isClick) => {
  ...
  LongTermNonReturnRepository.searchAll(request).then((function (response) {
    table.items = response.data.longTermNonReturnList
    table.totalElements = response.data.totalElements
    table.totalPages = response.data.totalPage
    table.amountOfDataDisplay = response.data.longTermNonReturnList.length

    let pagination = {
      page: table.options.page,
      totalElements: response.data.totalElements,
      totalPages: response.data.totalPage,
      amountOfDataDisplay: response.data.longTermNonReturnList.length,
    }
