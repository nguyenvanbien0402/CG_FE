<template>
  <div class="min-h-[85vh] flex flex-col relative">
    <div class="w-full p-5 border-b border-gray-50">
      <v-card :title="$t('dialogName.recordRegAppCancelDetail')" variant="flat"></v-card>
    </div>
    <div class="mb-15">
...
store.setBreadcrumb(store, {
  name: t('nav.screen.recordRegAppCancelDetail'),
  path:
    '/record-reg-app-detail/cancel?projectNo=' +
...
const getDetails = (request) => {
  RecordRegAppRepository.getDetails(request).then(function (response) {
    ...
    data.cancelNum1 = response.data.cancelNum1
    data.cancelNum2 = response.data.cancelNum2
    data.cancelNum3 = response.data.cancelNum3
    data.cancelNum4 = response.data.cancelNum4
    data.cancelRegDate = response.data.cancelRegDate
      ? response.data.cancelRegDate.replaceAll('-', '/')
      : null
  })
}
