<template>
  ...
  >
    ...
    >
      ...
      >
        <v-toolbar-title>
          {{ $t('dialogName.recordRegApp.singleRegisUpdate') }}
        </v-toolbar-title>
        <v-spacer/>
        <v-btn
          icon
...
const update = (request) => {
  RecordRegAppRepository.singleUpdateRegis(request).then(function (response) {
...
const getDataUpdate = (request) => {
  RecordRegAppRepository.getDetails(request).then(function (response) {
    data.request.regDate = response.data.regDate
