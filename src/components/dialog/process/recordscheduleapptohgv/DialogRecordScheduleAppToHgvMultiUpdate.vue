...
const open = (projectNo) => {
  ...
  data.request.returnDate = null
  data.request.groupHgv = null
...
const multiUpdate = (request) => {
  RecordScheduleAppToHgvRepository.multiUpdate(request).then(function (response) {
    if(response.code === 'ES201'){
      dialog.value = false
    }
