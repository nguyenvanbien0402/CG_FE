...
const multiUpdate = (request) => {
  RecordDeliveryRepository.multiUpdate(request).then(function (response) {
    if (response.code === 'ES200') {
      console.log('success edit')
      dialog.value = false
      emit('apply')
    } else {
      data.errorInfos.deliveryDate = ''
      data.errorInfos.showErrDeliveryDate = false
      response.data.forEach(error => {
        if (error.fieldName === "deliveryDate") {
          data.errorInfos.deliveryDate = error.errorCode
          data.errorInfos.showErrDeliveryDate = true
        }
      })
    }
  })
...
const close = () => {
  data.request.deliveryDate = null
  data.errorInfos.deliveryDate = null
  data.errorInfos.showErrDeliveryDate = false
