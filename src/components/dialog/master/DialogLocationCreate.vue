    ...
    <v-card class="rounded-lg">
      ...
      >
        <v-toolbar-title v-if="isAddNew">
          {{ $t('dialogName.master.location.add') }}
        </v-toolbar-title>
        <v-toolbar-title v-else>
          {{ $t('dialogName.master.location.edit') }}
        </v-toolbar-title>
        <v-spacer/>
        <v-btn
          icon
        ...
        >
          <v-container>
            ...
            >
              <v-col lg="2"
                     cols="12"
                     :class="data.errorInfos.showErrLocationId ? 'mt-3' : 'd-flex align-center'"
              >
                <span class="text-sm-left float-sm-left font-weight-medium text-sm">
                  {{ $t('locationType.locationId') }}
                  <span v-if="isAddNew" class="text-red-600">*</span>
                </span>
              </v-col>
              <v-col
                lg="10"
              ...
              >
                <v-text-field
                  ...
                  variant="outlined"
                  density="compact"
                  :hide-details="!data.errorInfos.showErrLocationId"
                  :error="data.errorInfos.showErrLocationId"
                  :error-messages="data.errorInfos.locationId"
                  type="number"
                  :max="'999999999'"
                  :oninput="validateInputLocationId"
                  :readonly="!isAddNew"
                  :class="!isAddNew ? 'bg-gray-200' : ''"
...
import {MAX_NUMBER_12_CHAR} from "@/constants/constant";
// Initial
...
const apply = () => {
  if (isAddNew.value) {
    ...
    let request = {
      locationId: idUpdate.value,
      locationIdRequestUpdate: data.request.locationId,
      name: data.request.name ? data.request.name : null,
...
const validateInputLocationId = () => {
  if (Number(data.request.locationId) > 9) {
    data.request.locationId = data.request.locationId.substring(0, 9)
  }
}
...
const addNew = (request) => {
  LocationRepository.add(request).then(function (response) {
    if (response.code === 'ES200') {
      console.log('success add')
      dialog.value = false
      emit('apply')
    } else {
      data.errorInfos.showErrLocationId = false
      data.errorInfos.showErrName = false
      data.errorInfos.name = ''
      data.errorInfos.locationId = ''
      response.data.forEach(error => {
        ...
        if (error.fieldName === "locationId") {
          data.errorInfos.locationId = error.errorCode
          data.errorInfos.showErrLocationId = true
        }
      })
    }
  })
...
const update = (request) => {
  LocationRepository.update(request).then(function (response) {
    if (response.code === 'ES200') {
...
const getDataUpdate = (request) => {
  LocationRepository.getById(request).then(function (response) {
