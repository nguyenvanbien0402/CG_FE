    ...
    <v-card class="rounded-lg">
      ...
      >
        <v-toolbar-title v-if="isAddNew">
          {{ $t('dialogName.master.contractType.add') }}
        </v-toolbar-title>
        <v-toolbar-title v-else>
          {{ $t('dialogName.master.contractType.edit') }}
        </v-toolbar-title>
        <v-spacer/>
        <v-btn
          icon
...
const addNew = (request) => {
  ContractTypeRepository.add(request).then(function (response) {
...
const update = (request) => {
  ContractTypeRepository.update(request).then(function (response) {
    if (response.code === 'ES200') {
...
const getDataUpdate = (request) => {
  ContractTypeRepository.getById(request).then(function (response) {
